...
fn check_authority(authority_info: &AccountInfo, sol: &SolData) -> ProgramResult {
    if !authority_info.is_signer {
        msg!("safe authority signature missing");
        return Err(ProgramError::MissingRequiredSignature);
    }
    if is_authority(authority_info, sol) {
        Ok(())
    } else {
        msg!("Incorrect safe authority provided");
    ...
    match instruction {
        SolInstruction::Initialize { size, init_data } => {
            ...
            if sol_address != *data_info.key {
                msg!("Error: safe address derivation mismatch");
                return Err(ProgramError::InvalidArgument);
            }

            let data_len = data_info.data.borrow().len();
            if data_len > 0 {
                msg!("safe account already initialized");
                return Err(ProgramError::AccountAlreadyInitialized);
            }

            let sol_signer_seeds: &[&[_]] =
                &[&authority_info.key.to_bytes(), br"safe", &[sol_bump_seed]];
        ...
        SolInstruction::Write { offset, data } => {
            ...
            if !account_data.is_initialized() {
                msg!("safe account not initialized");
